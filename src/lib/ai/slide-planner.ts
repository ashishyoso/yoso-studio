...
function plannerSystem(k: ClientKnowledge): string {
  return [
    ...
    `- IMAGERY: we have no approved asset files yet, so use source:"generate" for ALL imagery (never source:"asset"). Text must NEVER overlap an image — a clean text slide beats a crowded one. Rules:`,
    `   • Cover and SAVE slides: a friendly flat-vector mascot, placement "bottom-right".`,
    `   • Interior/section slides: a duotone photo bleeding off the BOTTOM (placement "bottom") ONLY when the body is SHORT — 4 or fewer short lines. Cool-blue duotone for the sleep topic, warm terracotta duotone otherwise.`,
    `   • If a section slide's body is 5+ lines OR it is stat/citation-dense, set image mode/source/placement = "none". No image.`,

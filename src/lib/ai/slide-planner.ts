...
function plannerSystem(k: ClientKnowledge): string {
  return [
    ...
    `- IMAGERY: we have no approved asset files yet, so use source:"generate" for ALL imagery (never source:"asset"). Match the reference decks:`,
    `   • Cover and SAVE slides: a friendly flat-vector mascot, placement "bottom-right".`,
    `   • Interior/section slides: a duotone photo bleeding FULL-WIDTH off the BOTTOM (placement "bottom") — like the reference decks. Cool-blue duotone for the sleep topic, warm terracotta duotone otherwise. The layout reserves the bottom band, so text sits above it (never overlapping).`,
    `   • Section slides essentially always get the bottom duotone photo (the layout auto-fits the text above it). Only set image "none" in the rare case of an extreme wall of copy (9+ lines).`,
    `   • The check/do slide NEVER has an image.`,

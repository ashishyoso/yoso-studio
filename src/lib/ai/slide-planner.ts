...
function plannerSystem(k: ClientKnowledge): string {
  return [
    ...
    `- IMAGERY (match the reference decks): like the real decks, most interior/section slides carry a duotone photo bleeding off the BOTTOM edge (placement "bottom", occupying roughly the lower quarter), with the text sitting above it. For the sleep topic use COOL-BLUE duotone; for other topics use warm terracotta duotone. The cover and save slide use a friendly flat-vector mascot (placement "bottom-right"). The check/do slide has NO image. Only set image "none" if a slide's body genuinely runs long (6+ lines) and would collide with a bottom photo. Text must never overlap the image.`,

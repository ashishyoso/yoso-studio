...
function plannerSystem(k: ClientKnowledge): string {
  return [
    ...
    `(B) FINISHED SLIDE SCRIPT (it contains per-slide markers like "SLIDE 1 · COVER", "HEADLINE", "SUB-PROMISE", "Here's what to check:", "CLOSER", or is clearly already broken into slides) → REPRODUCE IT FAITHFULLY. Map each provided slide to exactly one output slide, in order. Keep the EXACT wording of every headline, line, citation, and list item — do not rewrite, paraphrase, summarize, add, or drop anything. Your job is ONLY design: pick the layout, split copy into two-tone spans, set the thesis bar, format the check/do lists, and assign image specs. Use the "SLIDE N · <title>" text (minus the "SLIDE N ·" prefix) as the sectionHeader. The cover HEADLINE becomes the two-tone cover headline; SUB-PROMISE becomes the subhead. The CLOSER becomes the save slide (extract the comment KEYWORD and the offer). Preserve the slide count exactly.`,
    `   THESIS BAR (critical for faithful match): on each interior/section slide, the thesis bar MUST be the slide's FIRST declarative line — the opening sentence immediately after the header — reproduced VERBATIM. Do NOT synthesize or paraphrase a thesis, and do NOT also repeat that opening line in the body (it lives only in the bar). The body is the remaining lines.`,
    ``,
    ...
    `- IMAGERY: use source:"generate" for ALL imagery (never source:"asset"). The image MODE controls the layout, so VARY the mode across section slides to avoid a monotonous deck:`,
    `   • mode "duotone" → renders as a full-width photo bleeding off the BOTTOM. Cool-blue duotone for the sleep topic, warm terracotta otherwise. Best for lifestyle / statistic / India-context slides.`,
    `   • mode "anatomical" (or "3d") → renders as an illustration panel on the RIGHT (text flows in the left column). Best for mechanism / "how/why it works" / body-system slides.`,
    `   • mode "mascot" → a friendly flat-vector mascot. Use ONLY on the cover and SAVE slides.`,
    `   • ALTERNATE duotone and anatomical down the section slides (e.g. slide2 anatomical, slide3 duotone, slide4 anatomical, slide5 duotone…) so consecutive slides don't share a layout. Pick whichever fits the slide's topic, but keep the rhythm varied.`,
    `   • placement: set it to match the mode (duotone→"bottom", anatomical/3d→"right", mascot→"bottom-right"). The renderer enforces this regardless, but set it correctly.`,
    `   • The check/do slide NEVER has an image. Only use "none" on a section slide with an extreme wall of copy (9+ lines).`,
